import { ArrowLeft, ArrowRight, CheckCircle2, HelpCircle, LockKeyhole, ShieldAlert, Sparkles } from "lucide-react";
        ...
        <section className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-10 mt-8">
            <div>
              ...
              <div className="mt-10 p-6 rounded-2xl border border-white/10 bg-[#0D1120]"><h2 className="text-xl font-bold">How it works</h2><ol className="mt-4 grid sm:grid-cols-3 gap-5 text-sm text-white/60"><li><strong className="text-white block mb-1">1. Choose the system</strong>Review the outcome, included tools, and fit for your business.</li><li><strong className="text-white block mb-1">2. Complete checkout</strong>Secure payment is handled through Stripe once the product delivery destination is launch-ready.</li><li><strong className="text-white block mb-1">3. Receive instructions</strong>{deliveryCopy(product.fulfillmentType)}</li></ol></div>
            ...
            <aside className="lg:sticky lg:top-24 h-fit p-7 rounded-2xl bg-[#0D1120] border border-white/10">
              ...
              {checkoutReady ? null : <div className="mt-6"><button disabled className="w-full px-5 py-3.5 rounded-xl bg-white/5 border border-white/10 text-white/40 font-bold cursor-not-allowed inline-flex items-center justify-center gap-2"><LockKeyhole className="h-4 w-4" /> Checkout finalization in progress</button><div className="mt-4 p-4 rounded-xl bg-[#F59E0B]/10 border border-[#F59E0B]/25"><div className="flex items-start gap-3"><ShieldAlert className="h-5 w-5 text-[#F59E0B] shrink-0 mt-0.5" /><p className="text-xs leading-relaxed text-[#F8C866]">Approved pricing and Stripe mapping are complete. Checkout remains protected until this product’s customer-safe delivery destination passes final QA.</p></div></div></div>}
