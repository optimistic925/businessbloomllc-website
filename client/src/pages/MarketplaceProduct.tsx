...
import { getPublicMarketplaceProduct } from "../../../shared/marketplacePublicCatalog";
...
export default function MarketplaceProduct() {
  ...
  // Public pages do not receive internal fulfillment destinations or readiness details.
  // The server remains authoritative and currently blocks all Marketplace checkout until destinations pass QA.
      ...
      <main className="pt-28 pb-20">
        <section className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-10 mt-8">
            <div>
              <div className="flex flex-wrap gap-2"><span className="px-3 py-1 rounded-full text-xs font-semibold bg-[#7C3AED]/10 border border-[#7C3AED]/25 text-[#BDA4FF]">{product.category}</span><span className="px-3 py-1 rounded-full text-xs font-semibold bg-white/5 border border-white/10 text-white/60">One-time purchase</span></div>
              ...
              <div className="grid sm:grid-cols-2 gap-8 mt-10"><div><h2 className="text-xl font-bold">Who it is for</h2><p className="mt-4 text-white/65 text-sm leading-relaxed">{audienceFor(product.category)}</p></div><div><h2 className="text-xl font-bold">Product format</h2><p className="mt-4 text-white/65 text-sm leading-relaxed">Digital Business Bloom system delivered through the approved customer-access flow. No physical shipment is required.</p></div></div>
