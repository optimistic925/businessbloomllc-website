  ...
  return (
    <div className="min-h-screen bg-[#0B0F1A] text-white overflow-x-hidden">
      ...
      <main>
        ...
        <section className="py-20"><div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8"><div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4"><div><p className="text-[#BDA4FF] text-sm font-bold uppercase tracking-wider">Marketplace Collections</p><h2 className="mt-2 text-3xl font-black">Shop by business need</h2></div><a href="/marketplace" className="text-sm font-semibold text-[#14B8A6]">View Marketplace</a></div><div className="grid md:grid-cols-2 lg:grid-cols-4 gap-5 mt-8">{marketplaceCollections.map((collection) => <a key={collection.title} href={collection.href} className="p-6 rounded-2xl border border-white/10 bg-[#0D1120] hover:border-[#7C3AED]/40"><h3 className="font-bold">{collection.title}</h3><p className="mt-3 text-sm text-white/50 leading-relaxed">{collection.description}</p><span className="mt-5 inline-flex items-center gap-2 text-sm font-semibold text-[#14B8A6]">Browse collection <ArrowRight className="h-4 w-4" /></span></a>)}</div></div></section>

        <section className="py-20 border-y border-white/5"><div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8"><div className="grid lg:grid-cols-[1.2fr_1fr] gap-10 items-center"><div><Gift className="h-9 w-9 text-[#F59E0B]" /><h2 className="mt-5 text-3xl sm:text-4xl font-black">Start with a Free Resource</h2><p className="mt-4 text-white/60 max-w-2xl">Download practical worksheets, calculators, scorecards, spreadsheets, and checklists to get a focused quick win and identify the right paid system when you need deeper support.</p><a href="/resources" className="mt-7 inline-flex items-center gap-2 px-6 py-3 rounded-xl border border-white/10 font-bold">Browse Free Resources <ArrowRight className="h-4 w-4" /></a></div><div className="grid grid-cols-2 gap-4">{ALL_PUBLIC_FREE_RESOURCES.slice(0,4).map((resource) => <div key={resource.slug} className="min-h-32 p-5 rounded-2xl border border-white/10 bg-gradient-to-br from-[#F59E0B]/10 via-[#0D1120] to-[#14B8A6]/10"><span className="text-xs text-white/40">{resource.category}</span><p className="mt-3 font-bold text-sm">{resource.name}</p><p className="mt-2 text-[#14B8A6] font-black">FREE</p></div>)}</div></div></div></section>
