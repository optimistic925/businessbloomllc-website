      ...
      <main className="pt-28 pb-20">
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="max-w-4xl">
            ...
            <h1 className="mt-6 text-4xl sm:text-5xl lg:text-6xl font-black leading-tight" style={{ fontFamily: "var(--font-display)" }}>
              Practical systems to start, systemize, automate, and grow your business.
            </h1>
            <p className="mt-6 text-lg sm:text-xl text-white/60 max-w-3xl leading-relaxed">
              Explore current Business Bloom systems, toolkits, and resources built for entrepreneurs and growing businesses. Product pricing is approved; checkout remains protected until customer delivery is fully configured and tested.
            </p>
          </div>
          <div className="mt-8 flex flex-wrap gap-2">
            {categories.map((category) => (
              <a key={category} href={`#${categoryId(category)}`} className="px-4 py-2 rounded-full border border-white/10 bg-white/5 text-sm text-white/70 hover:text-white hover:border-[#7C3AED]/40 transition-colors">
                {category}
              </a>
            ))}
        ...
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-16">
          <div className="flex items-end justify-between gap-4 mb-8">
            <div><p className="text-[#14B8A6] text-sm font-bold uppercase tracking-wider">Current approved catalog</p><h2 className="text-3xl font-black mt-2">{MARKETPLACE_PRODUCTS.length} paid products</h2></div>
            <div className="text-sm text-white/45 hidden sm:block">{categories.length} active categories</div>
          </div>

          {categories.map((category) => {
            const products = MARKETPLACE_PRODUCTS.filter((product) => product.category === category);
            return (
              <div id={categoryId(category)} key={category} className="scroll-mt-24 mb-14">
                    ...
                    return (
                      <article key={product.slug} className="rounded-2xl bg-[#0D1120] border border-white/10 p-6 flex flex-col">
                        <div className="flex items-start justify-between gap-3">
                          ...
                          <span className="text-xs font-semibold px-3 py-1 rounded-full border text-white/60 border-white/10 bg-white/5">One-time</span>
                        </div>
                        ...
                        <div className="mt-5 flex items-end justify-between gap-4">
                          <span className="text-2xl font-black text-[#14B8A6]">{commercial?.priceDisplay ?? "Pricing unavailable"}</span>
                          <span className="text-xs text-white/40 text-right">Digital system</span>
                        </div>
        ...
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-20 rounded-3xl border border-[#7C3AED]/25 bg-[#7C3AED]/8 p-7 sm:p-9">
          <p className="text-[#BDA4FF] text-sm font-bold uppercase tracking-wider">Professional Systems™</p>
          <h2 className="text-3xl font-black mt-2">Buy individually or get the collection</h2>
          <p className="text-white/60 mt-3 max-w-3xl">Choose the system that solves the immediate problem, or use the complete collection for a coordinated operating-system approach across multiple business functions.</p>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mt-7">
            {professionalSystems.map((product) => {
              const commercial = getMarketplaceCommercialConfig(product.slug);
              return <a href={`/marketplace/${product.slug}`} key={product.slug} className="p-5 rounded-xl border border-white/10 bg-[#0D1120] hover:border-[#7C3AED]/40 transition-colors"><span className="font-semibold block">{product.name}</span><span className="text-[#14B8A6] font-bold mt-2 block">{commercial?.priceDisplay}</span></a>;
            })}
          </div>
